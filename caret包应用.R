...
# 自变量中还有可能存在多重共线性问题，可以用findLinearCombos命令将它们找出来。这样处理后自变量减少为94个。
comboInfo = findLinearCombos(newdata2)
newdata2=newdata2[, -comboInfo$remove]

# 我们还需要将数据进行标准化并补足缺失值，这时可以用preProcess命令，缺省参数是标准化数据，
# 其高级功能还包括用K近邻和装袋决策树两种方法来预测缺失值。此外它还可以进行cox幂变换和主成分提取。
...
# 可以选择的还有lmFuncs(线性回归)，nbFuncs(朴素贝叶斯)，treebagFuncs(装袋决策树)，caretFuncs(自定义的训练模型)。
...
models = list(gbmFit2)
predict(gbmFit2, newdata = testx)[1:5]
...
# plot( perf1 )
prob2$lable=ifelse(prob2$obs=='Active',yes=1,0)
pred2 = prediction(prob2$Active,prob2$lable)
